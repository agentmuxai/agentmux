...
pub struct AppState {
    ...
    /// comes from this host, not an agent process (agents share `auth_key`
    /// too, so that alone can't distinguish them — see
    /// `agentmux-srv/src/server/service/host_ipc.rs::handle_register`).
    /// Self-generated by default (host-owned-spawn/dev mode: this host
    /// mints it and passes it to srv itself at `spawn_backend` time, same
    /// as `auth_key`), overwritten from `AGENTMUX_HOST_REG_SECRET` when the
    /// launcher provided one (`use_launcher_endpoints`, launcher-managed
    /// mode — the launcher mints it once and gives the same value to both
    /// host and srv, so it survives host-only crash-restarts).
    pub host_reg_secret: Mutex<String>,
...
impl Default for AppState {
    fn default() -> Self {
        Self {
            ...
            host_reg_secret: Mutex::new(uuid::Uuid::new_v4().to_string()),
