...
**Date:** 2026-07-11
**Status:** active — Residuals 1 and 2 shipped the same day this was written, in the order §"Suggested PR slicing" recommends: Residual 2's `Window.meta["host:label"]` crumb and its cleanup fallback in #2094 (PR A), Residual 1's adoption of foreign `window-{uuid}` labels into the warm pool in #2102 (PR B). The crumb is written and read across `client/helpers.rs`, `window_pool.rs`, `server/service/window_create.rs` and `window_query.rs`; the adoption path names this spec and this residual in `cache_pool_window_view`'s doc comment. **Residual 3 is still open** — it asks for verification of the close path on macOS/Linux, and the code is still Windows-only by construction (`cache_pool_window_view` is `#[cfg(target_os = "windows")]`), so "believed" has not become "verified".
