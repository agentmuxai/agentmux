...
| **A pause longer than 90 ms mid-stream re-parsed the whole message twice** (the "settled" highlight flip invalidated #3559's frozen prefix) — the ~40 ms per-flush cost | Phase 2b: closed blocks always render at final quality; only the open tail follows `streaming`. fps 2.1–2.3×, key → paint p95 −33–37 %, script time ~4× lower | same tracker, §2.2 / §3.4 |

The remaining agent-pane work — cost that grows with conversation length — is planned in
