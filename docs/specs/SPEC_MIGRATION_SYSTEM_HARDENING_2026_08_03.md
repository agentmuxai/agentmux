...
**Date:** 2026-08-03
**Status:** active — Phase 0 shipped in PR #2394. Phase 1a (failure is fatal: srv emits `AGENTMUXSRV-MIGRATION-FAILED` and exits 1 before ESTART; launcher and CEF host fail fast with the reason; launcher shows the fatal dialog) shipped in #3043 + #3047. Phase 1b (`agentmux-srv migrate --verify` doctor pass: `Migration::verify` post-conditions, exit 3 on mismatch or unreadable tracking state, read-only end to end; real checks for 0007 (reusing `consolidate_looks_incomplete`) and the 0002 marker; 0003 deliberately not verifiable — its helper ignores the marker and re-asserts its invariant every startup) shipped 2026-09-07. Phase 3 (cross-process locks: dedicated SQLite lock dbs beside the shared store and beside the channel store, taken in that order, `BEGIN IMMEDIATE` held across check → run → mark on both the in-process and CLI apply paths; concurrent-runner tests incl. the different-shared-store case) shipped 2026-09-07. Phase 1c (`muxspect migrations` / `GET /api/v1/muxspect/migrations`: the same doctor report — applied/pending state plus every applied migration's verify verdict — served read-only from inside a live instance by the one producer `migrations::doctor_report` that `--verify` is now also built on) shipped 2026-09-07; Phase 1 is complete. Phase 6 (docs consolidation: a "What actually ships" section atop `SPEC_MIGRATION_FRAMEWORK_2026_06_24.md` superseding its subprocess-model description, plus the operator runbook `docs/recovery/RUNBOOK_MIGRATION_RECOVERY_2026_09_07.md`) shipped 2026-09-07. Phase 5 (the framework-level regression suite against the real registry — `agentmux-srv/src/migrations/phase5_tests.rs`: the stale-marker incident at the m0000 + runner level, crash-between-effect-and-mark resume, multi-version upgrade in registry order, and m0000_bootstrap stamping; the concurrent-boot item lives in `runner::lock_tests` from Phase 3) shipped 2026-09-07. Phase 2 (marker unification — `0002`'s helper no longer short-circuits on its flag and is content-idempotent (an archive is written only when no archive for that agent already holds the same bytes), `block_zones_look_incomplete` gates both m0000's stamping of `0002` and its verify the way `consolidate_looks_incomplete` already gates `0007`; the "no private marker files" rule is in `migrations/mod.rs`'s framework doc) shipped 2026-09-07 — acceptance met: no registered migration trusts a marker's existence as completion. Phase 4 not started (a product decision). Verified 2026-09-07.
...
**The `FOREIGN KEY constraint failed` warning that originally triggered this whole investigation is a separate, non-blocking bug**, not confirmed as Agent1's blocker (and per the above, likely unrelated to it). It's real and reproducible (a frontend flow labeled `linkagentidentity`, "direct identity link write-through", logs it repeatedly), caught and logged as `WARN`, non-fatal. **Correction (post-review, §0e below):** the specific mechanism first proposed here — a per-channel write-through mirroring into an empty `db_accounts` table — was checked against source and disproven; the actual code path producing this warning is not yet identified. Treat the warning as a confirmed real symptom with an unknown cause, not a diagnosed bug with a pending fix.
...
**New Phase 0e (added to the hardening plan in Part 3):** trace the actual code path behind the "direct identity link write-through" log line back to its real call site (start from the frontend log statement, not the backend RPC name — `linkagentidentity`'s own handler is confirmed not to be it) before designing any fix.
...
- **0e. Corrected after review (flagged by codex, verified here) — the proposed mechanism was wrong; the symptom is real but the root cause is now genuinely unidentified.** The original claim was that a per-channel "identity link write-through" mirrors `db_agent_identity_links` into each channel's `db_accounts`, hitting an FK violation because that table is empty per-channel. Verified against actual source: the `linkagentidentity` handler (`agentmux-srv/src/server/agent_handlers/identity.rs:518-539`) calls `Store::agent_identity_link` (`agentmux-srv/src/backend/storage/identities.rs:365-379`) — a single `INSERT ... ON CONFLICT` against whichever *one* store instance it's called on. **There is no dual-write, mirror, or per-channel write-through anywhere in this code path.** Separately, `id_store` — the store this handler actually writes through — is bound to the shared store, not the per-channel one, whenever `0011_shared_store_backfill` is applied (`bootstrap.rs:691-694`), which it was for the channel this was originally observed on. The shared store's copy of `db_agent_identity_links` has no FK on `account_id` at all (cross-DB FK enforcement isn't possible in SQLite), so this exact handler+store combination shouldn't be able to produce the observed error there either. **What's confirmed real:** the FK warning itself (`FOREIGN KEY constraint failed` on `linkagentidentity`, logged from the frontend as "direct identity link write-through failed") did fire repeatedly and reproducibly in a live instance. **What's not confirmed:** which code path actually produces it — plausibly a frontend-side write that bypasses this RPC entirely (e.g. a generic per-channel object-store write, not routed through `id_store`), but that's a guess, not a traced call site. This needs an actual trace (start from the frontend "direct identity link write-through" log site, not from the backend RPC name) before any fix is designed — no remedy is proposed here until the real path is found.
...
Acceptance: a synthetic test data dir with a stale `.flag` file and an empty `db_agents` table, run through `run_pending_migrations`, ends with `db_agents` correctly populated (not silently skipped). Separately for 0e: the actual call site producing the "direct identity link write-through" log line is identified and cited by file:line — this item isn't a code fix yet, its acceptance criterion is "the mechanism is known," full stop. Separately for 0f: after a clean boot with 0 pending migrations, the status-bar popover shows no "Migrations" row even if a prior launch in the same session had a nonzero count.
...
**Shipped 2026-09-07** — see the Status line; `agentmux-srv/src/backend/agent_session/migrations/v1_blocks.rs`, `m0000_bootstrap.rs`, `m0002_block_zones_v1.rs`, and the framework doc in `migrations/mod.rs`. The markers themselves stay on disk as evidence for pre-framework stamping and the doctor; retiring the files entirely is bundled with retiring the migrations (the "Retiring Old Migrations" rule).
...
**Shipped 2026-09-07:** items 1, 2, 3 and 5 are `agentmux-srv/src/migrations/phase5_tests.rs`; item 4 shipped with Phase 3 as `runner::lock_tests`.
...
`0a → 0b → 0c → 0d → 0e → 0f` (this week — 0a-0c/0f are cheap fixes closing the incident's root cause and the status-bar bug; 0e is now an investigation, not a fix, since its originally-proposed mechanism was disproven) → `Phase 1` (next — restores the spec's safety goal) → `Phase 5` tests written alongside 0/1 → `Phase 2` and `Phase 3` (can run in parallel once 0/1 are stable) → `Phase 6` docs as each phase lands → `Phase 4` on its own timeline since it's a scope/resourcing decision, not an engineering task, but should be raised explicitly rather than left to keep drifting.
...
Separately, low-priority: the `linkagentidentity` FK warning (§0e) is cosmetic today (swallowed, non-blocking) and doesn't need manual intervention — its cause isn't even identified yet (§0e was corrected after review; the originally-proposed per-channel-write-through mechanism was checked against source and disproven), so there's nothing to act on here beyond the Phase 0e tracing task. Same for the stale status-bar migration count (§0f) — refreshing/restarting clears the display even before the code fix ships.

**Superseded 2026-09-07 by `docs/recovery/RUNBOOK_MIGRATION_RECOVERY_2026_09_07.md`** (Phase 6). This section should be superseded by Phase 6's runbook once written — including a corrected version of the generic "stuck migration" recovery steps from the original (pre-Part-0) draft, which are still valid *if* the F1/F2 hypothetical in §1.2 is ever confirmed for a real instance, just not this one.
