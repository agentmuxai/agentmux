...
**Date:** 2026-08-03
**Status:** active — Phase 0 shipped in PR #2394. Phase 1a (failure is fatal: srv emits `AGENTMUXSRV-MIGRATION-FAILED` and exits 1 before ESTART; launcher and CEF host fail fast with the reason; launcher shows the fatal dialog) shipped in #3043 + #3047. Phase 1b (`agentmux-srv migrate --verify` doctor pass: `Migration::verify` post-conditions, exit 3 on mismatch or unreadable tracking state, read-only end to end; real checks for 0007 (reusing `consolidate_looks_incomplete`) and the 0002 marker; 0003 deliberately not verifiable — its helper ignores the marker and re-asserts its invariant every startup) shipped 2026-09-07. Remaining in Phase 1: wiring the verify report into muxspect (1c). Phases 2-6 not started. Verified 2026-09-07.
