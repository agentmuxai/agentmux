...
— #3237. Phase 3 (the availability/gating open question) is UNRESOLVED —
every agent gets these tools unconditionally today (option 1 from §2.4),
matching native memory's own precedent, but that was not an explicit
decision, just what shipping Phases 0-2 with no additional gating defaults
to. The read side of the audit trail, deferred from that pass, is built
too: #3448 added the `GlobalMemoryHistory`/`Diff`/`Revert` MCP tools, and
the Armory UI surfaces it over the `globalmemory:history`/`diff`/`revert`
WebSocket commands (`SPEC_MEMORY_FOLLOWS_THE_AGENT_2026_09_24.md` §2.3).
**Date:** 2026-09-15
