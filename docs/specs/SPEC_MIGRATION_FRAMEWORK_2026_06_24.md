...
**Date:** 2026-06-24  
**Status:** implemented — `agentmux-srv/src/migrations/` is the live framework (m0000–m0006+), used at every startup by `bootstrap.rs`; see the 2026-08-07 audit note below. Restamped 2026-09-06 (was "Proposed") per `docs/reports/REPORT_LARGE_MIGRATIONS_COMPLETION_AUDIT_2026_09_06.md` §5.3. — #3042
...
| **Concurrency** | Cross-process locks (Phase 3, #3062): `migrations.lock.db` beside the shared store AND beside the channel store (`<data-dir>/db/`), taken in that order, `BEGIN IMMEDIATE` held across check → run → mark on both the in-process runner and the CLI apply path; 30-minute wait; OS-released on holder death. Two because isolated-auth instances can share a channel store while resolving different shared stores. `--list`/`--dry-run`/`--verify` never take them. | `runner.rs::MigrationLock` |
