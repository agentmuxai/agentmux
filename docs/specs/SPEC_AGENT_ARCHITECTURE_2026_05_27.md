...
| 3b.5a | `agent_def_get` → `db_agents` | Low | ✅ shipped (this PR) | Sole production caller (`template_promote` idempotency check) uses a deterministic id format that won't collide with template-instance UUIDs. |
| 3b.5b | `user_clone_defs_for_template` → `db_agents` | Blocked | ⏳ deferred | Same schema ambiguity as 3b.3a: user-clone projections and template-instance projections share `is_template=0 AND parent_template_id=X` with no distinguisher. Function has **no live callers** — test-only — so deferral is low-cost. Address when a schema column distinguishes the two projections. |
| 3b.5c | `agent_def_delete_seeded` cascade-capture read | Blocked | ⏳ deferred | The read here captures cascaded INSTANCE ids before the legacy FK cascade fires, then drops projections via dual-write. It's a write-path helper that happens to read; not the kind of read 3b targets. Stays on legacy until Phase 3c retires the cascade pattern entirely. |
