...
> 5. **Drop**: both legacy tables, `agents_consolidate.rs`, `run_consolidate_migration`/`consolidate_looks_incomplete` (made tolerant of the tables' absence first, so `m0007`/`m0000` keep compiling and behaving safely); `0007` and its marker retired; `m0004`/`m0006`/`transcript_backfill` stay tolerant of `db_agents`-only stores. This doc's phase table is updated in each PR.
