...
| 3b.3a | `instance_list` (no `status` filter case) → `db_agents` | Low–Medium | ✅ shipped (this PR) | Frontend caller (`refreshInstances` in swarm-model.ts) passes `{}` and `instancesAtom` has zero readers in the live tree, so the user-facing surface is empty. `definition_id` filter matches `(id = ? OR parent_template_id = ?)` to honor both folded user-clone and template-instance lineages. Continuation chains pre-collapse to one row per logical agent. |
