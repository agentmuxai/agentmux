...
>
>    Two things it deliberately leaves for PR 3, both because `db_agent_identity_links` still has its FK on `db_agent_definitions`: an agent that exists only as a `db_agents` row cannot hold account links of its own (`listrecentsessions` falls back to its template's), and `agent_def_delete` still owns definition rows while `instance_delete` owns agent rows.
