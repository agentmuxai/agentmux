...
> 4. **Definition flip + drop**: the remaining `db_agent_definitions` reads/writes move to `db_agents`; `agent_def_delete`/`instance_delete` unify; both legacy tables, `dual_write.rs`, `agents_consolidate.rs`, and `repair_agent_def_gaps` are removed; `0007` and its marker are retired; `m0004`/`m0006`/`transcript_backfill` stay tolerant of `db_agents`-only stores. This doc's phase table is updated in each PR.
