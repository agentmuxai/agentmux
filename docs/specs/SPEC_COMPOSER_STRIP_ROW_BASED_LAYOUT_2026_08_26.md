...
| 21. Evicted stats float UP (user-directed) | ✅ Done | Same tier model as row 20, placement flipped: the evicted stats line renders ABOVE `.agent-composer-strip-rows` (DOM-order move in the column-flex strip), so as the pane narrows the centered token stats float up while the interactive slot rows stay anchored at the bottom next to the composer. CDP-verified with ticking stats: inline at wide, above the single slot row at the 2-line tier, above both rows at the 3-line tier, back inline on widening. |
