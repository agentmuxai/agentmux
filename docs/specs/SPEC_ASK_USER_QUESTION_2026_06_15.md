...
**Fix.** Give the **persistent** path the same resilience the one-shot path already has (§9 Phase 2): `answer_question` snapshots a stdout-frame counter (`stdout_seq`, an `AtomicU64` bumped at the top of the reader loop for **every** stdout line — including control frames the reader `continue`s past before `record_output`) **before** sending the `control_response`, then arms a short check (`ANSWER_RESUME_FALLBACK_MS`, 4 s). If the counter is **unchanged** by then, the turn did not resume → re-deliver the answer as a **directive follow-up user message** (`build_answer_resume_message`) on the live stdin, so the agent resumes with the decision in context. Because it is gated on stdout activity, it is **mutually exclusive** with a real resume and can never double-deliver. The directive wording ("Resume the task… do not wait for further input") also defends against the secondary failure where a vague "continue" nudge is read as a no-op.

> **Refinement (codex review on #1536, shipped):** the counter is snapshotted *before* the send (not after — closing a fast-resume race) and counts *all* frames incl. control (an earlier draft used `HealthMonitor::last_output_at()`, which skips control frames, so a resume whose first activity was a tool-permission round-trip could spuriously fire the fallback).

**Scope.** `agentmux-srv/src/backend/blockcontroller/persistent.rs` only (`answer_question` + `build_answer_resume_message` + `stdout_seq` counter + tests). No protocol, frontend, or `health.rs` change; the happy path (turn still live → `control_response` consumed) is byte-for-byte unchanged.
