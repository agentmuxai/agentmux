...
**Documentation gap, worth fixing alongside any code change:** both `bootstrap.ts:22,391` and `navigation.rs:17,57,277,311` reference the Linux startup paint-gating spec (added in 86693f8) (since removed from the tree) — and the commit message itself says "See ... for the full diagnosis and profiling data this is based on" — but **that file does not exist anywhere in the repository** (confirmed via `find`/`ls`). The diagnosis and profiling data this report would want to build on for a Windows equivalent was apparently never actually committed.
...
- Commit `2587b85c` (PR #2151, "fix(linux): gate startup window-show/splash-dismiss on real first paint") — the direct prior-art fix for §4.2, Linux-only; its own referenced spec (the Linux startup paint-gating spec (added in 86693f8) (since removed from the tree)) does not exist in the repo — a documentation gap worth closing alongside any Windows follow-up.
