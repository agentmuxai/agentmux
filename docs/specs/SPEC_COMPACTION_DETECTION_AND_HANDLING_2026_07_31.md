...
- New `agentmux-bashwrap precompact --trigger=manual|auto` subcommand (`agentmux-bashwrap/src/precompact.rs`): reads the `PreCompact` stdin payload (best-effort — a parse failure just means an empty `session_id`, never a crash), and if `AGENTMUX_LOCAL_URL`/`AGENTMUX_AUTH_KEY` are present, publishes a `compaction_started` WPS event (`{ trigger, sessionId, startedAt }`, camelCase, `persist: 1`) scoped to `block:<AGENTMUX_BLOCKID>`. Always exits 0 with no stdout output, regardless of outcome.
