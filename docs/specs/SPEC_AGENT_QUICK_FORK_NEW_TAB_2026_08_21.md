...
| **2** ✅ | Wire the quick-fork action itself. **Landed differently than originally drafted** — `pane.open` with an explicit `tab_id` (`open_pane`'s "explicit tab_id wins" path) creates+places the new block in one RPC, so no `waitForLayoutModel`/`createBlockOnModel` dance was needed; the launch itself reuses the SOURCE tab's own already-mounted `AgentViewModel` (via `getBlockComponentModel`) rather than constructing a new one, since `launchAgentDefinition`'s only per-instance state is `this.blockId` (already overridden by the new `targetBlockId` arg) and `this.nodejsError`. Also found and fixed a real prerequisite bug: `launchAgentDefinition` unconditionally used `atoms.staticTabId()` for `ControllerResyncCommand`'s `tabid` — harmless for the existing same-tab `targetBlockId` use case, but silently wrong for a genuinely cross-tab target (the backend stores this as the spawned controller's own registry `tab_id` and `AGENTMUX_TABID` env var, not just for logging) — added a `targetTabId` override param. Right-click trigger ("⑂ Quick-fork to new tab" in the tab's existing color/rename context panel) landed as part of this phase too, not deferred to Phase 3, since Phase 2 needs *some* way to invoke it to be a real, testable feature. | Phase 1 |
| **3** | UX polish: keybinding, auto tab title refinement, non-Claude fallback messaging, the tab-strip "+" split-button (§4.3's deferred item) | Phase 2 |
