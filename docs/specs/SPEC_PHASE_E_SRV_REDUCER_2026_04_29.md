...
| **E.2** | Workspace + Tab + ActiveTab arms. SQLite-bootstrap path **with replay-from-HWM (§6.1)**. Idempotent persist-subscriber (`upsert_*_if_version_higher`). Host bridge subscribes to srv pipe; renderer-side dispatcher routes srv events. RPC `dispatch_service` migrates these arms to go through the reducer (coexisting with bespoke WaveObjUpdate path). | ~900 |
