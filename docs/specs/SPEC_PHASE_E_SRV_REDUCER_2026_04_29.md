...
| **E.2** | Workspace lifecycle arms (`CreateWorkspace` / `DeleteWorkspace`). SQLite-bootstrap path for workspaces. Idempotent persist-subscriber that writes workspace events to SQLite. Existing HTTP/WS RPC continues to use wcore directly (bespoke path); the reducer is a parallel projection canonical for cross-process pipe-originated commands (saga coordinator in E.5+). dispatch_service migration + host bridge + renderer dispatcher deferred to E.2c. | ~500 |
| **E.2c** | Migrate HTTP/WS `dispatch_service::workspace.{Create,Delete}Workspace` to route through the reducer. Host bridge subscribes to srv pipe; renderer-side dispatcher routes srv events. Retire bespoke WaveObjUpdate for workspace lifecycle. | ~400 |
