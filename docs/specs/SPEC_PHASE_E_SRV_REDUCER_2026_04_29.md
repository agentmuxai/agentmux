...
| PR | Scope | Est LoC |
|---|---|---|
| **E.1a** | Saga coordinator infrastructure: `agentmux-launcher::saga` module, `Saga` trait, `SagaCoordinator` task, `SagaStarted`/`SagaCompleted`/`SagaFailed` events. **No actual sagas yet — just the framework.** Per-variant `saga_id` propagation on existing Command/Event variants is deferred to E.5 (where the first saga consumer needs it); E.1a uses the lifecycle events alone for correlation. Plus durable event-log hardening for the launcher (fsync per append) and the two codex P2 carryovers from PR #608 (identity-patch ordering + replay_truncated overflow). | ~500 |
| **E.1b** | New `agentmux-srv::reducer` skeleton: `State`, `Ctx`, `update` function with arms for `Register` / `Goodbye` / `Ping` / `GetSrvSnapshot` / `GetEvents`. Property tests. New srv pipe at `\\.\pipe\agentmux-{hash}\srv-command` + broadcast bus + event log (mirrors launcher's). srv main wires it up alongside the existing HTTP/WS server. **No host bridge or renderer dispatch yet** — those land in E.2 alongside the first domain events that need forwarding. | ~600 |
| **E.2** | Workspace lifecycle arms (`CreateWorkspace` / `DeleteWorkspace`). SQLite-bootstrap path for workspaces (one-time at srv startup). Reducer state is a session-only projection updated by pipe commands. **No persist subscriber yet** — existing HTTP/WS RPC continues to write to SQLite via wcore; pipe-originated commands (saga coordinator E.5+) only update reducer state. Persist subscriber lands in E.2c alongside RPC migration where it becomes essential. | ~400 |
| **E.2c** | Add idempotent persist subscriber. Migrate HTTP/WS `dispatch_service::workspace.{Create,Delete}Workspace` to route through the reducer (so pipe-originated commands also reach SQLite). Host bridge subscribes to srv pipe; renderer-side dispatcher routes srv events. Retire bespoke WaveObjUpdate for workspace lifecycle. | ~600 |
| **E.2b** | Tab lifecycle arms (`CreateTab` / `DeleteTab`) + `SetActiveTab` + `ReorderTabs`. SQLite-bootstrap extends to tabs. Same RPC migration pattern. | ~500 |
