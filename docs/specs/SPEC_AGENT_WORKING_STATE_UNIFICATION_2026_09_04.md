...
**DONE.** Gave the reducer a command that owns both signals atomically: `TurnEnd` now marks any still-`enqueuedWhileBusy` `pending` entries `flushing: true` in the same transition that flips `turnPhase` to `Done`, and `PendingMessagesPanel` renders "Sending — reaching the agent any moment" for flushing entries instead of "Queued — sends at the agent's next step" (which is user-facing-wrong the instant the turn ends). The panel still disappears only once the async ack (`PendingMessageAccepted`/`Rejected`) actually clears the entry — the fix closes the window where the *label* already changed but the *copy* still claimed a state that no longer applied, not when the panel disappears. Implemented in `reducer.ts`/`state.ts`/`PendingMessagesPanel.tsx`, covered by 4 new reducer tests + 5 new component tests.
