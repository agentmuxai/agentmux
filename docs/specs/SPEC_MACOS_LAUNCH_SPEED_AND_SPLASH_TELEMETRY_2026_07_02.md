...
4. B.4.1-6 (macOS: thread the receiver, stage-list rendering, hold, selftest wiring) — the visible "load times in splash" deliverable. Also the natural place to add the pre-supervisor stage (item 2's deferral) for all three platforms together, since it requires moving `StartupEventSink` creation earlier in `main()` — exactly the restructuring macOS needs anyway to have a sink to give the splash.
5. A.4.2/A.4.4 (binary-path caching, IPC-signal dismiss) — smaller, independently landable cleanups. A.4.5 moved to item 3′.
