...
1. **Parallelize srv spawn and host spawn instead of ESTART-gating host start** — `unix.rs:289-338` (mirrors `windows.rs:355-420`). Today host spawn (and its own multi-second CEF init) doesn't even *begin* until srv has fully booted. The host only needs srv's endpoints once it's past its own CEF init and ready to call the backend — host spawn could start concurrently with srv spawn, gating only the point where the host actually needs a live backend connection (poll/retry on not-yet-ready backend for the first second, or block only the specific first RPC call). **Largest single opportunity** — likely saves several hundred ms to low seconds depending on how much CEF init and srv init currently overlap in wall clock (today: zero overlap, fully serialized). Applies identically to Windows and Linux — should be fixed once, cross-platform, in `unix.rs`/`windows.rs`.
2. **Cache/derive the host binary path instead of a runtime `read_dir` scan** — `binary_resolution.rs:8-47`. The versioned filename is knowable via `env!("CARGO_PKG_VERSION")` at compile time; resolve directly in the common case and only fall back to the directory walk if that exact path is missing. Small absolute win (single-digit ms) but removes blocking syscalls from the critical path for free.
3. **Feed `StartupEventSink` into the macOS splash** (`unix.rs:204-208` currently drops `rx`) — doesn't itself speed up launch, but is the instrumentation prerequisite for measuring (and then optimizing) macOS phase timings, and is the direct dependency of Prong B. See §B.
4. **Replace the splash's file-existence busy-poll with a real signal** — `splash_mac.rs:371` polls `std::fs::metadata` every 8ms for up to 10s (`DISMISS_TIMEOUT`). Minor CPU/wake overhead, not a big latency win, but a clean opportunity to instead have the host write to the launcher's already-open Unix-domain IPC socket on first paint, removing the temp-file protocol and the poll interval's few-ms tail latency.
...
6. **CEF `dlopen` cost is real but is the hard-constraint floor (§A.3)** — once instrumented (Prong B closes this gap), if profiling shows this dominating, the only levers are CEF's own (`--disable-features`, prewarming) or accepting it as the floor. Flagging here so a future spec doesn't propose removing/reordering it.
...
1. Correct CLAUDE.md's stale dev-mode claim (§0) — trivial, unblocks accurate onboarding.
2. B.4.1 + B.4.7 (thread the receiver, add `"host"`/pre-supervisor stages) — instrumentation prerequisite, cross-platform benefit, no rendering work yet.
3. A.4.1 (parallelize srv/host spawn) — biggest win, cross-platform, now measurable via the stages just added.
4. B.4.2/3/4/5/6 (macOS stage-list rendering + hold + selftest wiring) — the visible "load times in splash" deliverable.
5. A.4.2/A.4.4/A.4.5 (binary-path caching, IPC-signal dismiss, saga/spawn overlap) — smaller, independently landable cleanups.
