...
| **P1b** ✅ | **clear/delete**: per-session `history.Delete` + bulk `history.Clear` (provider/project filter); removes the native transcript **and** Claude's sibling `<sid>/` subagents dir | **DONE** — the "clear" requirement, with unit tests. **Mirror descoped — see note.** |
| **P1c** | Codex + Gemini `HistoryAdapter`s (scan isolated `CODEX_HOME`/`GEMINI_CLI_HOME`) | Multi-provider browse + clear |
...
> **Mirror descoped (2026-06-10):** §4 proposed re-copying every transcript into `shared/history/`. On inspection that's **low value for high cost** — provider histories already live **inside** AgentMux's home (`shared/providers/<p>/`, not GC'd by the CLIs), so a duplicate would double ~540 MB (Claude alone) for marginal durability. Instead: P1a makes them browsable in place, P1b adds clear/delete in place, P1c extends to more providers. The remaining real sub-goals of the mirror — **orphan migration** (pull pre-isolation `~/.claude` sessions into the isolated home; P0 already does this on resume) and **agent-anchored grouping** (defragment cwd-slug splinters) — are better as targeted features than a blanket copy. Revisit a normalized (not raw) mirror only if cross-machine export (P3) needs it.
