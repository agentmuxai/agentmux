...
(Attempts 1-3 — the live repro this spec fixes). Surfaced while building a
separate, not-yet-landed UI-manual screenshot tool — not cited by path
...
- **CEF profile / data directory** — `agentmux-common/src/data_paths.rs`'s
  ...
  **deliberate**, not a bug: dev mode's own comment says its data-dir
  ...
  `RuntimeMode::Dev{branch, clone_id}`, the resolved directory is
  `dev/<branch>/<clone_id>/` — keyed on **branch AND `clone_id` together**,
  not branch alone (corrected after ReAgent P1 on this PR's implementation
  — an earlier revision of this spec claimed branch alone, which this
  session's own repro can't actually distinguish from "same branch, same
  clone path," since every attempt in the linked report ran from the same
  checkout). `clone_id` (`runtime_mode.rs`'s `derive_clone_id`) is a hash
  of the clone's own canonical workspace-root path — explicitly
  git-worktree-aware, so a second worktree of the SAME branch already gets
  a distinct `clone_id` and therefore a distinct data dir, no branch
  switch required. No version component enters this path at all either way
  (`resolve_internal`'s `version_dir` is only version-scoped for
  `Installed`/`Portable`, not `Dev`).
...
**second** (CEF's own `SingletonLock`, inside the `(branch, clone_id)`-keyed
data dir) — which is the one that actually matters for "can two instances
run at once." **A second instance launched from the identical checkout on
the identical branch can never work via a version bump** — the real fix is
a different branch, OR a different clone/worktree path (either one changes
`clone_id` or `branch`, and therefore the resolved directory).
...
1. **`second_instance.rs`** (both call sites — the fast-path collision and
   ...
   keyed on branch AND clone/workspace path together, a version bump
   doesn't help, either a different branch or a different clone/worktree
   path resolves it — and include the actual `data_dir` path in the
   message (it's already an argument to the function, just wasn't being
   printed).
...
- **Not** changing the actual isolation model ((branch, clone_id)-keyed
  data dir, version-keyed launcher socket) — that's a bigger, riskier change
