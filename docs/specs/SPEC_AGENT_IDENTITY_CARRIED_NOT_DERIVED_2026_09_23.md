...
| `agent.open` (`app_api/agent_open.rs`) | writes `agentId` and spawns, but never calls `instance_create`: a user agent with a local row resolves; a cross-channel agent with no local row, or a template, does not | **M4b-4**: `agent.open` records **every** launch through `instance_create` and stamps the block with the resulting row (`agentInstanceId`), so a template opened this way is a real, row-backed agent inside the gate (Codex P1 on #3578: excluding it let the gate clear with live tokenless template agents, which M5 would strand). Three rules (Codex P1s on #3578): **(a) row first, identity from the row.** For a fresh open of a template, `instance_create` runs *before* the pane's config is built, and the config takes its **identity fields** — `AGENTMUX_AGENT_ID`, the slug (collision-resolved, `claude-2`), the UID and token — from the created row, because `build_persistent_spawn_env` does not replace an existing `AGENTMUX_AGENT_ID`; built from the template, the process would carry the new row's token while routing and signing as the template. Its **configuration** — content, instructions, environment, skills, MCP servers — still comes from the template, as today: `instance_create` copies the row's scalar columns but not `db_agent_content` or the skill and MCP bindings, which `write_agent_config_files` looks up by id, so building everything from the new row would launch a blank agent (Codex P1 on #3578). **(b) Reopen folds, never re-creates.** On the reuse path (`open_agent_impl`, the `existing` branch — a block found for the agent and resynced, e.g. after a restart), a block whose row still resolves (#3576) folds into that row; **a block with no resolvable row gets one** — a fresh template block, and equally a template block `agent.open` created before M4b (no stamp, no launch row), which would otherwise stay tokenless outside the gate after every restart (Codex P1 on #3578). Test: reopening such a legacy template block creates, stamps and resolves a row. Otherwise every restart would mint a new identity for the same pane and leave the old row `running`. **(c) Both branches, before either resyncs**, including the reuse path for a user agent: a cross-channel block created before M4b has no local row and, reopened after a restart that way, would otherwise spawn tokenless where the gate cannot see it. A user agent folds into its own row, backfilling a cross-channel definition as a picker launch does. **(d) A fold never blanks bindings.** `instance_create`'s fold overwrites `identity_id`, `memory_id`, `working_directory` and `instance_name` with the caller's record, and `agent.open` supplies none of them — so folding into an existing row is a **lifecycle-only** update (`last_block_id`, `status`, `started_at`, as the fold moves them), never `instance_create` with a fresh record; `instance_create` is used only where a row must be created (a block with no resolvable row, a cross-channel backfill). Otherwise a reopen would clear the agent's account, bundle and workspace (Codex P1 on #3578). |
| Template-based continuation (reattach of a record whose `definition_id` is a template) | `SetMeta{agentId: template}` → resync (eager resume) → `CreateAgentInstanceCommand` → `SetMeta{agentInstanceId}`. At the eager resume the block's stamp is **the previous launch's** (`backToPicker` clears `agentId`, not `agentInstanceId`), and a same-template sibling's stale `running` row still sits on the block: the resume can bind to **the sibling** — its UID, token, slug and credentials, invisible to every counter because it carries a token | **M4b-3 (P1)**: `CreateAgentInstanceCommand` first, **and it must succeed**: on failure the launch aborts with the error in the pane — it never resyncs without a row (Codex P1 on #3578); then the pane's config (`cmd:env`, `.mcp.json`) takes its **identity fields from the returned row** — its collision-resolved slug, not the template's — and its configuration from the same source as today, as in M4b-4 (a) (Codex P1s on #3578); then **one** `SetMeta` carrying `agentId`, the returned `agentInstanceId` and that env; then resync. Setting `agentId` mounts the agent view, whose own launch flow resyncs — so the stamp must land in the same `SetMeta`, never after. `backToPicker` clears `agentInstanceId`. Tests: a store test with a stale stamp and a same-template sibling on the block; a frontend test that the create and the stamp precede any resync. **Recorded:** if the resync then fails, the row is left folded onto a block that never ran it (status `running`) — as for a launch that crashes. In practice this path is rare: local rows reattach as themselves; only legacy registry records naming a template reach it. |
