...
| `/btw` side question (`side_question.rs`) | a throwaway block with no `agentId`: no row | outside for M4b, `spawn.no_row.subprocess` — **pending #3579**: if the `/btw` CLI can reach agentmux-mcp as the pane's agent, it should carry the pane block's UID + token instead; decided there, before M4d. |
...
gate nothing: they are the declared-outside set (terminals, `/btw` pending
