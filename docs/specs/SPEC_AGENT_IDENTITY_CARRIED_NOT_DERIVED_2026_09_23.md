...
| Path | Today | M4b |
|---|---|---|
| App API `agent.send` (`app_api/agent_io.rs`) | a hand copy of `run_agent_turn`'s env: `cmd:env` + `inject_identity_env_async` only | **M4b-1**: env from `build_persistent_spawn_env`, as `agentinput`. **And the builder makes `AGENTMUX_AGENT_ID` server-authoritative:** when the block resolves to a row, it is set from the row's slug, as `AGENTMUX_AGENT_SLUG` already is, overriding `cmd:env` and the `agentName` fallback — identity no longer depends on a meta the mount flow can replace (adversarial P2 on #3578). The `.mcp.json` copy the MCP reads is written separately, and its drift is #3573. Same spawn gate (its only error is `inject_identity_env_async`, which `agent.send` already calls). **Also changes, recorded:** the process gains `AGENTMUX_AUTH_KEY`, `AGENTMUX_BLOCKID`, PATH, `MUXBUS_TOKEN` and the server-authoritative slug and display name, and git authorship becomes `<slug>` / `<slug>@agentmux.local` — what `agentinput` gives today. Containers keep PATH off (denylist). Internal respawns reuse the triggering send's config, so they carry too. |
| `agent.open` (`app_api/agent_open.rs`) | writes `agentId` and spawns, but never records the launch: a user agent with a local row resolves; a cross-channel agent with no local row, or a **template** (opened as a template block), does not | **M4b-4**, two rules. **(a) A template is opened the way the picker opens one:** `agent.open` of a template runs the picker's create-from-template (`agentdefcreatefromtemplate`: a new user agent cloned from the template — its content, skills and MCP bindings — with its own collision-resolved slug and its own working directory, allocated at first launch) and then opens *that agent*, so its block's `agentId` is a real row id and the first branch resolves it. No template block is ever created again. This replaces revision drafts that created a launch row under a template block, which an adversarial pass showed shared one working directory across opens (each row's `.mcp.json` overwriting the last), left the reuse branch's config naming the template while the process carried the new row's token, and let the new row hijack later name lookups into duplicate panes. Recorded: each open of a template creates a user agent, as a picker launch does; a later `OpenAgent <name>` resolves to the most recent agent of that name, as for any user agent. **(b) A user agent's open records its launch** on both of `agent.open`'s branches, before either resyncs — the new-block flow and the reuse path (`open_agent_impl`'s `existing` branch) — through a **lifecycle-only** update (`last_block_id`, `status = running`, `started_at`, `ended_at = 0`; a new store call, since `instance_update_partial` cannot set `started_at`), never `instance_create` with a fresh record, whose fold would overwrite `name`, `identity_id`, `memory_id`, `instance_name`, `github_context` and `user_hidden` from a record `agent.open` does not have. It stamps the block with the row id. A cross-channel agent with no local row is backfilled first (`instance_create`'s definition backfill), and its config is built after the backfill, from the local row — the backfill can collision-rename the slug. **Legacy template blocks** (opened as templates before M4b) are not reached by either rule after a restart — the agent view's own launch flow resyncs them, not `agent.open` — so they are counted as **row-expected** (below), keeping the gate open until they are closed or reopened. Test: a reopen that folds into a row with an account, bundle and workspace leaves them unchanged and the spawn uses them; an `agent.open` of a template yields a new user agent and a block whose `agentId` is its id. |
| Template-based continuation (reattach of a record whose `definition_id` is a template) | `SetMeta{agentId: template}` → resync (eager resume) → `CreateAgentInstanceCommand` → `SetMeta{agentInstanceId}`. At the eager resume the block's stamp is **the previous launch's** (`backToPicker` clears `agentId`, not `agentInstanceId`), and a same-template sibling's stale `running` row still sits on the block: the resume can bind to **the sibling** — its UID, token, slug and credentials, invisible to every counter because it carries a token | **M4b-3 (P1)**: `CreateAgentInstanceCommand` first, **and it must succeed**: on failure the launch aborts with the error in the pane — it never resyncs without a row (Codex P1 on #3578); then `.mcp.json` is written with the **returned row's** identity — its slug, read back from the row (an `AgentInstance` carries none), not the template's — while its configuration comes from the same source as today (Codex P1s on #3578); then **one** `SetMeta` carrying `agentId` and the returned `agentInstanceId`; then resync. The process's `AGENTMUX_AGENT_ID` is not put into `cmd:env` — the agent view's mount flow replaces `cmd:env` wholesale — but set by the server at spawn from the resolved row (M4b-1). Setting `agentId` mounts the agent view, whose own launch flow resyncs — so the stamp must land in the same `SetMeta`, never after. `backToPicker` clears `agentInstanceId`. Tests: a store test with a stale stamp and a same-template sibling on the block; a frontend test that the create and the stamp precede any resync. **Recorded:** if the resync then fails, the row is left folded onto a block that never ran it (status `running`) — as for a launch that crashes. In practice this path is rare: local rows reattach as themselves; only legacy registry records naming a template reach it. |
...
`live.tokenless_or_unknown` is zero. A block is **row-expected** when it
resolves to a row, *or* its `agentId` names an agent definition — a template
or any known definition id — without resolving: a legacy template block, a
pane whose agent was deleted. Row-expected blocks count toward
`spawn.no_token` and `live.tokenless_or_unknown`, not `no_row` /
`unidentified`, so the gate stays open until such panes are closed or
reopened (adversarial P1 on #3578). `spawn.no_row.*` and `live.unidentified`
gate nothing: they are the declared-outside set (terminals, `/btw` pending
#3579, quick-launch). A continuation whose create fails aborts (M4b-3), so it
is not in that set.
...
existing row carrying an account, bundle and workspace, then asserts the
row's bindings are unchanged **and** the rebuilt `cmd:env` / `.mcp.json`
use them (the row's slug, account and working directory), for a user agent
and for a template-backed row (ReAgent P1 on #3578).
