...
2. **Throwaway prototype, not a shipped feature**: verify `tray-icon`+`muda` actually coexists with CEF's event loop on Windows and macOS specifically (the one genuinely unverified architectural assumption in this whole design) before committing further engineering to the approach.
