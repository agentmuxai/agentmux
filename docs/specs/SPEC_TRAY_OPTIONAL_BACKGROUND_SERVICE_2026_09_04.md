...
- ~~**Opt-in and disabled by default.** Never silently enabled by an installer or an update.~~ **Superseded 2026-09-25 by the repo owner: the tray / background mode is on by default** (`app:runinbackground` defaults to `true`). The rest of this section still holds, and the icon is what makes the default acceptable: a resident process must always show one. If the tray is requested but cannot start (e.g. Linux without a StatusNotifier host), the launcher spawns the host without background mode, so closing the last window quits (`tray::unavailable`, `host_spawn::drop_background_if_no_tray`). Auto-start stays a separate, opt-in toggle.
...
| WS1 Linux (`ksni`) | Done (`tray/linux.rs`, see `SPEC_OS_NOTIFICATIONS_SYSTEM_2026_09_24.md` §11.5). If no StatusNotifier host is available the tray fails to start and background mode is dropped for the host | — |
