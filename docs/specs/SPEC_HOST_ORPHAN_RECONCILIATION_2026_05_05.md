...
Drain is gated on `safe_to_drain = live_user_count == 0 && freshly_promoted.is_empty()`. The second clause matters because a freshly-promoted candidate (Race B) is a live user window whose `WindowOpened` echo hasn't returned yet — it's NOT in shadow, so it doesn't bump `live_user_count`, but draining around it would freeze its session (`QuitState` has no transition back from `Draining` to `Running`, so `spawn_pool_window` permanently refuses pool refills). When `safe_to_drain` is false, the reconciler closes any zombies in `to_close` and returns; the next `HostShouldQuit` (after shadow catches up, or after the live windows actually close) resolves the situation.
