...
| **4** ✅ | `.pane-tab-strip` width transition (§2.4). | low | CSS-only: `interpolate-size: allow-keywords` + `transition: width 160ms ease-out` on `.pane-tab-strip` (`PaneTabStrip.scss`) — lets the browser animate the strip's own shrink-to-fit width across a tab-count change with no JS measurement. Relies on this app's pinned, modern bundled Chromium (CEF 148) rather than a cross-browser-safe technique. Covered by this app's existing global `prefers-reduced-motion` transition reset (app.scss) — no local override needed. |
