...
| **2** ✅ | Wired the leaf-scoped gate into all three callers — `handleNewAgentTab` (`agent-view.tsx`), `openOrFocusHistoryTab` (both its create-new and switch-to-existing paths), `quickForkAgent` — each holding on its own pane's node id before its first RPC and scheduling the lift in a `finally` covering every exit path (success, RPC failure, pane-closed-mid-flight, and — for quick-fork — the launch itself rejecting). Tile renderer reads `gatingNodeIds()` per leaf, merged into the existing `tileTransform()` style object (not a wrapper element, to leave each leaf's absolute positioning untouched) in all three platform `TileLayout.*.tsx` files identically. | med | Mitigated the "leaf gated forever" risk via the try/finally pairing (mirrors `tab-actions.ts`'s own established pattern) — every caller's finally block covers all its exit paths, verified by dedicated tests per caller. |
