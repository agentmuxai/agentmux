...
| `BrowserDispatchKey(key, modifiers?, text?)` | `dispatch_key` | Types into whatever currently has focus — the actual "without a mouse" input path for text fields. **Corrected post-review (reagent P0, PR #3445):** this table originally claimed dispatch_key was "safe for both Path 1 and Path 2... CDP already scopes correctly today" — true only when `selector` is given (routed through the existing `__amq_focus`/`__amq_allowed_for` ownership check). When `selector` is omitted, keys land on whatever currently has focus on the resolved page — for a Path 2 (shared-window) target that's whatever ANY pane currently has focused, not necessarily the caller's own. Fixed by adding an ownership check (`__amq_focus_owned_by`) on the no-`selector` path: rejects rather than dispatching blind when the focused element isn't the caller's own. |
| `BrowserFocusElement(selector)` / `BrowserFocusInfo()` | `focus_element`/`focus_info` | `focus_element` was already correctly scope-aware. **`focus_info` was NOT** (reagent P0, PR #3445, caught the same review pass as the dispatch_key issue above): the existing route had a comment explicitly justifying its lack of scoping as "not currently reachable via any agent-facing MCP tool" — a justification this very PR invalidates by adding `BrowserFocusInfo`. Unfixed, it would have let any agent read the tag/text/attrs/rect of whatever element is focused ANYWHERE in the shared window, including another pane's active input. Fixed by threading `block_id` through to `__amq_focus_info`, applying the same `__amq_allowed_for` ownership check `query`/`click_element` already use. |
