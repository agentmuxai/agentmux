...
**Date:** 2026-09-24
**Status:** active — Phase 1 (`/quit` + `/exit`, `sagas/self_quit.rs`) shipped in PR #3779; Phase 1b (visible shutdown log) shipped in PR #3784; Phase 2 (`QuitSelf` + gate) and Phase 2b's `QuitSelf` half (`sagas/pending_shutdown.rs`, banner, tone) in PR #3789; 2b's `ClosePane block_id=` / `FleetBulkStop` half in PR #3798; Phase 3 (no-argument `ClosePane` → self-quit behind the window) in PR #3802.
