...
## 8. Remediation shipped (2026-08-14, same session)

PR [#2575](https://github.com/agentmuxai/agentmux/pull/2575) implements items 1 and 2 above, scoped deliberately narrow — **not** a rearchitecture. Both land in `frontend/app/view/agent/hooks/useTurnLifecycle.ts`, mirroring that file's own existing `stopFallbackTimer` pattern (a reactive `createEffect` on `turnPhase.kind`, not a new event-consumption mechanism):

- A `createEffect` arms/clears a real `SUBMIT_TIMEOUT_MS` `setTimeout` while `turnPhase.kind === "Submitting"`, dispatching the reducer's own `SubmitTimeoutElapsed` command on expiry (going through the already-tested, documented `Submitting → Done.errored` contract, not a hand-rolled shortcut).
- A `document.visibilitychange` listener fires one extra `StreamWatchdogTick` the instant the document regains visibility — a backstop for the interval that this investigation found no evidence was ticking. This directly targets the confirmed AgentA failure mode without inventing new backend/RPC surface: `visibilitychange` fires reliably even for throttled content (browsers rely on it to drive that exact throttling), and `StreamWatchdogTick`'s recovery math is already wall-clock-based (not tick-counted), so one catch-up tick is sufficient regardless of how long the interval was stalled.

Verified: `tsc --noEmit` (zero new errors — the 2 pre-existing errors were confirmed present on a clean `main` checkout too), the full `reducer.test.ts` suite (191/191, unchanged — these fixes don't touch `reducer.ts`), a new `useTurnLifecycle.test.ts` (6 new tests covering both fixes directly), and the full `frontend/app/view/agent` suite (1149/1149, no regressions).

**Deliberately not done in this PR** (per the §7 ranking and the "no architecture rethink needed, narrow structural fix instead" scoping decision): item 3's `INTERRUPT_TIMEOUT_MS` gap was investigated further and found to be **not actually a live bug** — `useTurnLifecycle.ts` already has a working, if inconsistent, fallback (`stopFallbackTimer`, a 1.5s local timer that calls `finalizeTurn` directly, bypassing the reducer's own unused `InterruptTimeoutElapsed` contract entirely). Since `Interrupting` doesn't currently hang, "unifying" it to go through the same contract as the new `Submitting` fix would be a pure refactor with real regression risk on a working path and no bug driving it — left undone, documented here as a known follow-up rather than shipped speculatively.
