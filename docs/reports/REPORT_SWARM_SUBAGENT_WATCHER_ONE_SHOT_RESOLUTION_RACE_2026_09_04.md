...
### Second review round: the exact same failure class reappeared one layer up

ReAgent's re-review caught that `recheck_all_watched_agents()` — which runs on *every* `agent_identity_link` write anywhere in the app, with no scoping to the agent whose identity actually changed — iterated *every* `WatchedAgent`, including ones registered via that same legacy empty-`parent_block_id` RPC. A blind re-resolve against `""` (or any block that's since been deleted) falls through to the same `derive_claude_config_dir` last-resort guess and silently repoints an agent that was never supposed to be identity-resolved at all — the identical bug the P1 fix's first attempt had already been caught and fixed for *inside* `watch_agent`, reappearing at the "recheck all" layer above it. Also flagged a stale doc comment on `start_watch` describing a return value `watch_agent` no longer consumes (leftover from the reverted first-attempt design).

Fixed by skipping any candidate in `recheck_all_watched_agents` with an empty `primary_block_id` or one that no longer resolves to a real `Block` — only ever recheck an agent whose config dir could legitimately have come from identity resolution in the first place. Simplified `start_watch` back to returning `()` since nothing consumes its result anymore. Two new tests, each falsified against a reproduction of the exact bug before being confirmed correct.

## Verification

- 9 new tests: no-op when unwatched, no-op when unchanged, repoint replaces the entry, repoint preserves multiple dependent blocks, repoint backfills a session's subagents missed while watching the wrong directory, repoint preserves the *exact* original `primary_block_id` across 6 dependent blocks (not an arbitrary set member), repoint keeps the old watch when the replacement fails to build, `recheck_all_watched_agents` skips the legacy empty-block_id entry point, `recheck_all_watched_agents` skips an agent whose block no longer exists.
- Falsified four times: (1) neutered `recheck_config_dir` to an early return, confirmed exactly the 3 original behavioral tests fail; (2) reproduced the P2 bug (remove-before-build ordering), confirmed the new "keeps the old watch on failure" test fails; (3) the P1 self-check design error was caught live by the pre-existing suite itself, not a deliberate falsification — see above; (4) removed the `recheck_all_watched_agents` skip-guard, confirmed both new skip tests fail with exactly the wrong (derived-fallback) directory. Every case restored, diffs clean.
- Full `subagent_watcher` suite: 96/96 (87 pre-existing + 9 new), confirming the three-way split (`build_watch`/`start_watch`/`spawn_consumer_loop`) is behavior-preserving.
- Full `agentmux-srv` suite: 3018/3018 passing, zero regressions.
