...
1. **Don't collapse auth popups into a pane's frame — let CEF create a real child popup.** In `on_before_popup`, detect the popup case (`NEW_POPUP`/`NEW_WINDOW` disposition) for browser panes and **return false so CEF creates an actual popup browser** in the pane's own request context. This was chosen over the simpler "route to the system browser" alternative because that alternative, while it stops the crash, **breaks the login**: a separate browser process has no `window.opener` relationship to the pane and doesn't share cookies, so GIS's `postMessage` credential hand-off never reaches the pane ("There was an error logging you in"). A real child popup shares the context — `window.opener`, `postMessage`, and cookies all work, so the login **completes in the pane** — and its own `window.close()` closes only the popup, leaving the pane and main window untouched (the instance-exit bug was the pane's OWN frame being navigated + self-closed; a distinct popup closing is normal). Ordinary `target="_blank"` links (tab dispositions) still navigate in-pane, unchanged. **Implemented (PR #2545).**
2. **Add an `on_before_browse` / external-protocol guard to the pane RequestHandler.** Block any non-http(s) scheme a pane tries to navigate to. Defense-in-depth that stops *any* embedded content — not just Google — from reaching an OS handler that can raise a prompt, and makes the offending scheme visible in our logs. **Implemented (PR #2545).**

**Note on the "UAC":** the firewall prompt the user saw ("CEF Bootstrap Application") was later root-caused to Chromium's mDNS listener binding `UDP 0.0.0.0:5353` (a Windows Defender Firewall "allow access" dialog, not a true elevation UAC) — fixed independently by disabling `WebRtcHideLocalIpsWithMdns` in **PR #2547 (merged)**. The external-protocol handoff hypothesized above is guarded by fix 2 regardless, but was not the source of the dialog the user reported.

3. **Harden bug 2 so a pane close can never strand the main window hidden (deferred, defense-in-depth).** Two parts: (a) ensure the airspace-hide **restore** is not owned by an individual pane's lifecycle — on any pane destroy, unconditionally restore main-window visibility before the quit watchdog can sample; and (b) in the watchdog's `registered>0 && !draining` branch, treat "a registered window exists but is only *hidden* (not closed)" as **do-not-quit**. **Not implemented** — fix 1 already breaks this incident's chain (the pane no longer self-closes), and this touches the delicate last-window-quit path (documented prior P1 regressions), so it's filed as a scoped follow-up rather than changed blind.

Fixes 1 + 2 (PR #2545) and PR #2547 together resolve the user-visible incident — crash gone, login completes in-pane, no firewall prompt. Fix 3 remains a latent-defect follow-up.
