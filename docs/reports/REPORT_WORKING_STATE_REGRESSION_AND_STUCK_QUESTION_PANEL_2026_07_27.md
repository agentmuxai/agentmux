...
**Tests:** backend — `dead_transition_publishes_unresponsive_failure` and `dead_recovery_clears_the_unresponsive_failure` (`health.rs`, both drive `Dead` via `record_error(Fatal, ...)` since the 120s silence branch isn't practical to wait out in a test, and verify the publish/clear via `Broker::read_event_history`). Frontend — a new `failureToRow` case test (Restart is primary, no plain Retry offered) and two new `useAgentFailure.ts` tests (null-data event clears an `unresponsive` row; does NOT clear an unrelated one).
