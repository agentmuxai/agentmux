...
**Fix (implemented):**
- `frontend/app/view/agent/hooks/useAgentQuestions.ts` — `handleAnswer`'s fallback is no longer gated on a specific error string; it now retries via the follow-up-message path on **any** `AgentAnswerCommand` failure, only rolling back if that fallback itself also fails. This is strictly safer than the narrow check: whatever the specific reason the control-protocol path failed, redelivering via the ordinary message-send path is a reasonable next step — and that path (`send_message` on the backend) already auto-spawns/resumes a process if one isn't running, so this also self-heals a genuinely dead process without requiring a manual pane reopen at all, not just the "reopened but stale in-memory record" case.
- `agentmux-srv/src/backend/blockcontroller/persistent.rs` — `answer_question`'s "not found" error message now explicitly names the likely cause (process respawn) for `muxlog` diagnosability, though the frontend fix no longer depends on the exact string.
- Tests: `useAgentQuestions.test.ts` (new — success path keeps the optimistic state with no fallback; a non-`UNSUPPORTED_CONTROLLER` failure now falls back and keeps the optimistic state; a failure where the fallback *also* fails still rolls back) and `answer_question_on_untracked_tool_use_id_is_descriptive` (backend, asserts the error message names the tool_use_id and the likely cause).
