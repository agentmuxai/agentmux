...
confirmed on a real run: 55,367/55,367 ninja targets, zero errors (see
"Status" below).
...
**Status (2026-07-27): validated end-to-end with a real build.** The steps
below produced a working `libcef.dll` — 55,367/55,367 ninja targets, zero
compile/link errors, boot-verified via `cefsimple.exe --url=https://example.com`,
and published as [`cef-windows-x86_64-148.0.7778.180`](https://github.com/agentmuxai/cef/releases/tag/cef-windows-x86_64-148.0.7778.180)
on `agentmuxai/cef`. Along the way this surfaced and fixed several real gaps
the first-pass draft didn't anticipate — worth knowing before you hit them
again on a future build:
- The `src` checkout can drift off the CEF-pinned Chromium revision if
  `gclient sync` runs without a revision pin — pin explicitly to the tag in
  `cef/CHROMIUM_BUILD_COMPATIBILITY.txt` (`refs/tags/148.0.7778.180` for this
  branch) before syncing, or CEF's patches fail to apply (115 patches, 55
  failed in the drifted state observed live).
- `cef/tools/patcher.py` takes **no arguments** in its normal mode (reads
  `cef/patch/patch.cfg` itself) — there is no `--root-dir` flag.
- GN required args should come from `python3 cef/tools/gn_args.py windows`
  (`GetRequiredArgs()`/`GetRecommendedDefaultArgs()` in that script), not
  hand-guessed — `args-windows.gn` originally omitted
  `enable_cdm_host_verification`/`enable_cdm_storage_id`/`enable_rlz`/
  `alternate_cdm_storage_id_key`, all required by `cef/BUILD.gn`'s own
  `assert()` block on Windows.
- `build/util/LASTCHANGE.committime` isn't generated by any `gclient` hook —
  it's just the pinned commit's raw Unix timestamp
  (`git log -1 --format=%ct <hash>`), write it directly if `gn gen` complains
  it's missing.
- Two Windows toolchain components aren't part of a default VS Build Tools +
  Windows SDK install and need adding separately: **Debugging Tools for
  Windows** (Windows SDK installer → `/features OptionId.WindowsDesktopDebuggers`,
  needed for `dbghelp.dll`) and **C++ ATL/MFC support**
