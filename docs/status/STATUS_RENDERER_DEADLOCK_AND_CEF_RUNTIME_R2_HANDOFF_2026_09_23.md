...
- Agent credentials: the Agent3 GitHub App token CAN create releases on `agentmuxai/cef` even though `GET repos/agentmuxai/cef` reports `push=false` for it. The `.gh-env.sh` PAT in the Agent3 workspace is dead (401) and the `gh-token-agent3` / `gh-token-genericagentx` secrets no longer exist. For cross-repo writes, get Agent3's App token for that account with the installed `gh-agent` CLI: `gh-agent token --owner agentmuxai` (run as Agent3). The repo-local `scripts/gh-agent.sh` and `scripts/github-app-token.py` this note originally cited have since been removed.
