...
- Three independent review rounds across two PRs caught three genuinely different bugs (a fallback-shape gap, a scrub-path gap, a false-positive-tagging gap) that manual testing alone would likely have missed, since none of them show up unless you specifically construct the harness's less-common response shapes.
...
- Done: `muxspect dock`'s `bg` column gives a human (or another agent) a fast way to spot this class of issue without transcript spelunking, next time something adjacent breaks.
...
- No general process exists yet for `git log origin/main..HEAD` sanity-checking before every push — it's a manual habit this retro reinforces, not a hook. A pre-push check (local git hook or a `task` target) would remove the manual step entirely.
