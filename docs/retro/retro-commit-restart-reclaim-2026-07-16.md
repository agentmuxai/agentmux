...
3. **Add a manual `--disable-gpu` override** (setting or env var). Today the switch only auto-fires when commit-free < 512 MB at startup (`app.rs:747`). A user on a commit-tight machine has no way to opt into software rendering ahead of pressure, and support/diagnosis has no way to A/B it without editing code.
