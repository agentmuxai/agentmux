...
3. **Explicitly rejected: a manual `--disable-gpu` override.** The correction spec's §A.4 proposed it as the decisive attribution test; this experiment already answered that question, so the diagnostic justification is gone. As a *mitigation* it's counterproductive: SwiftShader software rendering allocates CPU shared memory that is itself pagefile-backed commit (see the `window_pool.rs` "SwiftShader = CPU shared memory = pagefile-backed commit" comment, ~90 MB+/window measured), so it degrades the whole UI while plausibly not reducing commit at all. Owner decision 2026-07-16: not wanted. The existing automatic <512 MB startup gate stays as-is.
