...
| **E.2c.3** | Tab RPC migration. **Carryover: codex P2 #615** — `wstore_workspace_exists` masks `StoreError`, returns false on read errors. Propagate `Result<bool, StoreError>` and surface in DeleteWorkspace. Then migrate Tab.{Create,Delete,SetActive,Reorder}Tab through reducer | |
