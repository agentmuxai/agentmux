...
| **E.2c.3b** | CloseTab + ReorderTab migration + pinned-tab support in reducer (`pinned_tab_ids` field on WorkspaceRecord, bootstrap split, subscriber routing, `CreateTab.pinned` command field). **Carryovers from #616**: (a) pinned-tab activate sync should use `Some(pinned_tab_id)` not `None`, (b) propagate StoreError in pinned-tab membership detection in SetActiveTab | |
