...
Critically, **this machine was running 12 other AgentMux processes** (other
agents' dev/portable instances — `tasklist | grep -c "agentmux-0.53"` → 12) at
the time of all three failures, on top of the build's own 32-wide C++
compile. That's the resource-contention half of the story: 32-way MSVC
parallelism is already aggressive for a shared PDB; adding a dozen
already-running AgentMux instances competing for the same CPU/disk/handle
budget makes the PDB-writer's synchronous-access window far likelier to be
starved long enough to trip C1083.
...
`tasklist | grep -c "agentmux-0.53"` → **12** concurrently running AgentMux
processes at the time of both failures. `wmic cpu get
NumberOfLogicalProcessors` → **32**, matching the `--parallel 32` cmake
invocation exactly (i.e. the build was requesting full-machine parallelism
with zero headroom for the other 12 processes already using it).
...
- Multiple agents on this repo normally work on disjoint code in parallel
  ...
  workflow.
- Nobody had previously run `task dev`/`task package` on this machine while
  12 sibling AgentMux instances were already up. The isolation model
  documented in `CLAUDE.md` ("Multiple Instances Run in Parallel") guarantees
  data-dir/port/binary isolation between *running* instances — it says
  nothing about, and doesn't protect, concurrent *build* CPU contention.
...
racing). Recommended default on machines that regularly run multiple
AgentMux instances concurrently.
...
| 4 | Later, asked to pull latest `main` and produce a fresh portable. `task package` hit the identical C1083 signature a third time, same `libcef_dll/*/test/*.cc` batch, same 12 concurrent sibling AgentMux processes. |
| 5 | Retro written instead of a fourth blind retry, per explicit direction ("lets get a retro written to file..we should be clear for this stuff"). |
