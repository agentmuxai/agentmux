...
- **2026-04-04, `e3a6f85c2` ("fix: kill shell processes when closing windows and tabs", PR #299)** — `close_window` in `agentmux-srv`'s wcore cascades workspace → tabs → blocks and kills their shell processes on close, to stop orphaned shells piling up in Task Manager. Legitimate fix for a real leak.
