...
The codebase has been mostly cleaned up from the Wave Terminal → AgentMux migration, but several categories of dead code and legacy naming remain. The AI panel (`aipanel/`) is **DEAD CODE** — the sidebar AI chat is not used. The agent pane (`view/agent/`) handles all AI agent interactions. See `docs/specs/archive/remove-aipanel-sidebar.md` for full removal plan.
...
Removal touches 12+ files across workspace layout, focus manager, keyboard shortcuts, and RPC layer. Full removal spec: `docs/specs/archive/remove-aipanel-sidebar.md`
