...
A follow-up ask widened scope to also cover **runtime resource cost** — added bundle size, load time, memory, and installer/package size — covered in §7, alongside the engineering-time and translation-vendor cost in §5–§6.
...
Of 147 `.scss` files, **29 files (57 declarations) use physical `left:`/`right:` positioning; a separate, distinct file elsewhere in the tree uses a logical property** (`margin-inline`-style) — none of the 29 already does it correctly. RTL support means auditing and converting all 29 of those files, plus wiring the HTML `dir` attribute and auditing directional icons — none of which is currently in place anywhere in the codebase.
...
| RTL | HTML `dir` attribute + CSS logical properties (`margin-inline-start` instead of `margin-left`) | Matches finding in §3.5 almost exactly — all 29 offending files need this conversion. RTL is not "flip the characters"; layout, icon mirroring, and native-speaker/device testing are all required — simulator RTL behavior diverges from real devices. |
...
| Plural/number/date formatting engine | **Native `Intl.PluralRules`/`Intl.NumberFormat`/`Intl.DateTimeFormat`**, not a JS-shipped ICU MessageFormat library (`@formatjs/intl-messageformat`) | Zero-byte marginal cost specific to this app — see §7. A JS ICU parser is a real, previously-reported bundle-bloat contributor elsewhere; AgentMux's CEF runtime already bundles full Chromium ICU data for other reasons, so the native APIs are free where a JS implementation would not be. |
...
## 7. Resource footprint: bundle size, load time, memory, package size

This section costs the *runtime* footprint of i18n itself — separate from the engineering-time and translation-vendor cost in §5–§6. Where no citable published benchmark exists for a specific library, that's stated explicitly rather than presented as a measured figure; the right next step before committing to a design is measuring the actual pinned dependency versions with `vite-bundle-visualizer`/Bundlephobia (frontend) and `cargo bloat` (Rust), not trusting this section's estimates as final.

**Baseline, directly observed in this repo today:** this session's own `task package` build (v0.55.38, 2026-09-06) produced a **431 MB extracted portable / 185 MB zipped** artifact. It's dominated by the bundled CEF/Chromium runtime — `libcef.dll`, `.pak` resource files, the V8 snapshot, SwiftShader software-GL fallback, and `icudtl.dat` (Chromium's full ICU data file) are all copied into `dist/cef/` as part of that build, per the packaging script's own bundling steps. Every figure below should be read against that baseline.

| Dimension | Estimate | Basis |
|---|---|---|
| **Per-locale translation dictionary (frontend)** | ~25–70 KB gzip per locale (≈120–240 KB uncompressed JSON, at an assumed ~80 bytes/entry for ~1,500–3,000 keyed strings) | Stated estimate, not measured — JSON of short repeated-structure UI strings compresses well under gzip; exact figure depends on final string count from Phase 2. |
| **i18n library JS (`@solid-primitives/i18n`)** | Low single-digit KB gzip | `solid-primitives` packages are characteristically minimal (thin composable primitives, not a framework); no citable exact figure found — measure the pinned version directly before finalizing. |
| **Plural/date/number formatting** | **~0 additional bytes** | Recommend native `Intl.PluralRules`/`NumberFormat`/`DateTimeFormat` (§4) over a JS ICU MessageFormat implementation. The ICU data backing native `Intl` already ships today inside the bundled Chromium runtime (`icudtl.dat`, confirmed present in this repo's own build output above) for reasons unrelated to i18n — using it costs nothing incremental. A JS-shipped alternative is not free: FormatJS's own GitHub issue tracker has reports of its ICU message-format parser contributing over 100 KB unminified in real-world projects — described in one report as heavier than the rest of the library combined — before tree-shaking/gzip and FormatJS's own precompiled "no-parser" optimization bring it back down. |
| **Native (Rust) host strings** | Well under 1 MB across the ~50–70 strings × illustrative locale count, baked into the binary at compile time | Fluent's `static_loader!` macro compiles `.ftl` resources directly into the binary (confirmed in `fluent-templates` docs) — no runtime file I/O, but binary size grows (trivially, given the string count) with every locale added rather than being fetched on demand. No published binary-size-overhead benchmark exists for the Fluent crate family; recommend `cargo bloat` before/after as part of Phase 0. |
| **Load time — best case** | Sub-millisecond to low single-digit ms added to cold start | If only the active locale's ~25–70 KB dictionary loads (true lazy-loading), this is noise against a CEF/Chromium app's existing multi-hundred-ms cold start. |
| **Load time — worst case, and the one that matters** | Grows **linearly with every locale added**, all of it paid on every launch regardless of active locale | Directly gated by §3.6's finding: `manualChunks` is disabled repo-wide, so today all `import()`-loaded code inlines into one bundle rather than splitting into separate per-locale files. Unless Phase 6 (§5) delivers true per-locale lazy-loading, all 4 illustrative locales' dictionaries ship and parse on every cold start — a German user's launch pays for Japanese and Arabic strings too. This is a real, quantifiable cost of *not* resolving the Phase 6 architecture question, not a hypothetical one. |
| **Memory** | Low single-digit MB for one active locale's parsed dictionary (V8 object overhead typically runs 2–4x raw JSON bytes) | Immaterial against a Chromium multi-process renderer's existing baseline (commonly 50–150+ MB per window even without i18n). Not expected to be measurable in practice. |
| **Package/installer size (net)** | **Well under 0.25% of the current 431 MB baseline**, even in the worst uncompressed, all-locales-shipped case | 4 locales × ~70 KB (frontend, high end) + a few KB (library) + well under 1 MB (native, all locales) totals well under 1 MB against a 431 MB portable build. Not a driver of ship/no-ship decisions at any point in this rollout. |

**Bottom line: bundle size, memory, and package size are not meaningful constraints on this project at any phase — they're rounding error against AgentMux's existing CEF/Chromium footprint.** The one dimension that *is* a real, non-trivial cost is **load time under the worst-case bundling path**, and it's entirely a function of the same §3.6/Phase 6 architecture decision already flagged as this report's #2 schedule risk (§8) — resolving it well (true per-locale lazy-loading) makes the load-time cost negligible too; resolving it poorly (ship everything inlined) makes it grow with every future locale, indefinitely, as a permanent tax on every user's cold start regardless of their language.

## 8. Risks and open questions, ranked by how much they could move the estimate
...
2. **The WebKitGTK/`manualChunks` constraint (§3.6) could add real time to Phase 6** if the selective-re-enable path is needed — that path directly re-tests conditions that caused a past regression on Linux. It's also the one architecture decision with a real, ongoing resource-cost consequence rather than a one-time engineering cost — see §7's load-time analysis.
...
- [@solid-primitives/i18n — Bundlephobia](https://bundlephobia.com/package/@solid-primitives/i18n)
- [Optional intl-messageformat-parser · Issue #1945 · formatjs/formatjs](https://github.com/formatjs/formatjs/issues/1945)
- [fluent-templates — GitHub (XAMPPRocky)](https://github.com/XAMPPRocky/fluent-templates)
- [ICU4X — GitHub (unicode-org)](https://github.com/unicode-org/icu4x)
