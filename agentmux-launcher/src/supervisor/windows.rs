...
pub(crate) async fn run_windows(
    ...
    if let Some(tray_rx) = tray::start_if_enabled() {
                ...
                while let Ok(action) = tray_rx.recv() {
                    match action {
                        tray::TrayAction::OpenWindow => {
                            // Exactly the path a second launch / the macOS
                            // reopen delegate uses — see design doc §7.5.1.
                            match second_instance::forward_host_cmd(
                                &tray_data_dir,
                                &tray_dir_hash,
                                "open_new_window",
                            ) {
                                Ok(()) => log("tray: open_new_window forwarded"),
                                Err(_) => log("tray: open_new_window forward failed"),
                            }
                        }
                        tray::TrayAction::Quit => {
                            // Graceful: `quit_app` drains with
                            // LauncherRequested so srv still gets its
                            // per-window cleanup. Killing the job object here
                            // instead would leak srv rows and resurrect ghost
                            ...
                            match second_instance::forward_host_cmd(
                                ...
                                Ok(()) => log("tray: quit_app forwarded"),
                                Err(_) => log("tray: quit_app forward failed"),
                            }
                        }
                    }
                }
