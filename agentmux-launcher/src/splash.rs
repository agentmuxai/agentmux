...
unsafe fn run_splash(
    ...
    loop {
        // Drain all pending startup events (non-blocking) as one batch —
        // `apply_tick` defers a same-tick Begin+End pair to the next call so
        // every row gets at least one "running" frame before it can
        // complete, instead of snapping straight to its final value (the
        // count-up bug; see `splash_core`'s doc comment — this used to be
        // macOS-only, this loop had its own copy of the pre-fix code).
        ...
        if WaitForSingleObject(dismiss_ev, 0) == WAIT_OBJECT_0 {
            ...
            loop {
                ...
                let drained_any = !fresh.is_empty();
                timeline.apply_tick(fresh);
                let still_open = timeline.stages.iter().any(|s| s.done.is_none());
            ...
            // Any row still open past the drain window above — e.g. the
            // `first-run-wait` sub-row, which has no `SubEnd` call anywhere
            // in the codebase — gets a definite (Warn-tagged) outcome here
            // instead of freezing mid-"running" for the whole hold + fade.
            // See `StageTimeline::finalize_running`'s doc comment.
            timeline.finalize_running(Instant::now());

            // Freeze: render final state with brain steady.
            composite(dib_pixels, 200u8, &timeline.stages, Some(total_ms), &footer, restoring);
            push_layered(hwnd, mem_dc, 255);

            // Summary hold — 2 s default, shortened for very fast starts.
            let hold = hold_duration(total_ms);
            if !hold.is_zero() {
...
fn draw_stages(dib: &mut [u8], stages: &[StageEntry], total_ms: Option<u64>, restoring: bool) {
    ...
    // phase. `other` is new here (splash_core::reconcile) — Windows
    // previously showed a bare, unreconciled "total" with no accounting of
    // the gap between it and the sum of the rows above; macOS has carried
    // an equivalent "other" row since PR #2244. Both platforms now compute
    // it identically via the same shared function.
    // docs/reports/REPORT_SPLASH_SCREEN_ARCHITECTURE_RETHINK_2026_09_14.md §4.
    if let Some(ms) = total_ms {
        let (_accounted, other_ms) = reconcile(stages, ms);
        if row + 1 < MAX_STAGE_ROWS {
            let y = y0 + row as i32 * STAGE_ROW_H;
            let other_str = format!("other: {}", format_ms(other_ms));
            let tx = x_time_right - text_width(&other_str);
            draw_text(dib, SPLASH_W, SPLASH_H, tx, y, &other_str, SUB_COLOR, 1.0, true);
            row += 1;
        }
