...
// (docs/specs/SPEC_LAYOUT_FILES_2026_09_25.md §3.5 "Trust"): what tabs and
// panes it adds, what can't be reproduced here, and every terminal command
// in it — which run only if the user leaves "Run these commands" ticked. The
// box starts ticked only for a trusted file (saved by this install, in its
// layouts folder).
...
export interface LayoutPreviewModalProps {
    ...
    onOpen: (runCommands: boolean) => Promise<void>;
    close: () => void;
}
...
export function LayoutPreviewModal(props: LayoutPreviewModalProps): JSX.Element {
    ...
    const tabCount = () => props.preview.tabs.length;
    return (
        <ConfirmModal
            open
            title={`Open layout “${props.preview.name || "Layout"}”`}
            description={`Adds ${tabCount()} ${tabCount() === 1 ? "tab" : "tabs"} to this window. Nothing that's open is replaced.`}
            confirmLabel="Open"
            onConfirm={async () => {
                await props.onOpen(runCommands());
                props.close();
            }}
