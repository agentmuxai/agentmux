 ...
 * is an async RPC round-trip). Buffering + retrying that case used to
 * live here as a local retry keyed off observing `turnPhase`, then a
 * time-bounded version of the same — both were structurally unsound (PR
 * #2928 review, reagent P1 + codex P2 twice over): a hook observing only
 * the RESULTING `turnPhase` value cannot reliably tell "this promotion is
 * the SAME turn the buffered ping was about" from "this is a later,
 * unrelated turn," and cannot react to an authoritative
 * `ReconcileTurnActive(active: false)` confirmation that happens not to
 * change `turnPhase` at all. That buffering now lives in the reducer
 * itself (`state.pendingCompactionPing`, `reducer.ts`'s `CompactionStarted`
 * / `ReconcileTurnActive` / `StreamFlushObserved` / `TurnStart` cases),
 * which sees every discrete command regardless of whether it changes the
 * resulting state — this hook stays a plain dispatch-and-check, same as
 ...
 * command as a LIVE compaction (i.e. it set `state.compacting` directly,
 * not merely buffered it onto `state.pendingCompactionPing` for possible
 * later promotion — see `reducer.ts`'s `CompactionStarted` case and
 * SPEC_COMPACTION_STARTED_RECONCILIATION_RACE_2026_09_02.md). Pure and
 * exported for direct unit coverage (reagent P1, PR #2378 round 6):
 * without this check, a rejected-or-merely-buffered ping would still get
 * a permanent "Compacting conversation…" transcript node pushed here,
 * even though the reducer hasn't (yet, or ever) confirmed it's real.
 */
