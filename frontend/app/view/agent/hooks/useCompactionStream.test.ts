...
describe("wasCompactionStartedAccepted", () => {
    ...
    it("returns false when only a buffered (not-yet-promoted) event is present", () => {
        // SPEC_COMPACTION_STARTED_RECONCILIATION_RACE_2026_09_02.md: a ping
        // rejected while turnPhase isn't working yet is buffered onto
        // state.pendingCompactionPing and reported via a DISTINCT event
        // ("compaction-started-buffered", not "compaction-started") — the
        // hook must not push a transcript node for it until the reducer
        // actually promotes it (a separate dispatch's "compaction-started").
        expect(
            wasCompactionStartedAccepted([{ type: "compaction-started-buffered", trigger: "auto" }]),
        ).toBe(false);
    });
});
