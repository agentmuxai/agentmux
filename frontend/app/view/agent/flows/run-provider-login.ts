...
export async function runProviderLogin(p: RunProviderLoginParams): Promise<ProviderLoginOutcome> {
    ...
    // not just Claude — so tier 2 and tier 3 share the SAME account instead
    // of each minting its own. Account minting itself (ensureAccountDir /
    ...
    // reaching this function just gets `null` back here. A prior version of
    // this code additionally gated the CALL on `provider.id === "claude"`
    // — reagent P0: that meant a successful tier-3 terminal login for
    // codex/openclaw never minted or persisted a real IdentityAccount at
    // all, so the agent reported "Login successful" but stayed permanently
    // blocked by the resolver's unconditional oauth-class spawn gate on its
    // very next spawn. existingAccountId threads through so a Reconnect
    // (not a fresh Connect) refreshes the SAME account's dir.
