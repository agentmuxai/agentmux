...
pub(crate) async fn open_agent_impl(
    ...
    tracing::info!(agent_id = %cmd.agent_id, "agent.open");
                ...
                // 8b. Point this agent's row at the block we just created,
                // BEFORE resyncing.
                //
                // Every other launch path records the launch through the
                // `createagentinstance` RPC, which the frontend fires after
                // `agent.open`. This path had no equivalent, so the block was
                // handed to `resync_controller` with nothing in `db_agents`
                // tying the agent to it. That was invisible until eager resume
                // started gating on launch-specific evidence
                // (`try_eager_resume` -> `instance_get_by_block_id`): the
                // lookup keys on `last_block_id`, which nothing here ever set,
                // so it returned `None` every time and eager resume could
                // never fire for an App API / MCP reopen — silently disabling
                // the feature on this whole call path, and with it the
                // real-status branch below. Found by codex (P2) and then
                // reagent (P1) on PR #3523.
                //
                // `instance_create` is the right call despite the name: for a
                // user agent (`is_seeded == 0`) it FOLDS onto that agent's own
                // row rather than inserting, so this repoints `last_block_id`
                // and is idempotent with the frontend's own
                // `createagentinstance` landing on the same row moments later.
                //
                // Carried forward from the existing row with `..existing`
                // rather than built fresh, and this is load-bearing: the fold
                // writes `identity_id`, `memory_id`, `instance_name`,
                // `working_directory` and `github_context` UNGUARDED (only
                // `last_block_id` and `session_id` are `CASE WHEN ?= ''`
                // protected). A naively-constructed row would therefore wipe
                // the agent's bound account and memory bundle — a far worse
                // regression than the one this fixes. Only `block_id` changes.
                //
                // `agent.id`, not `cmd.agent_id`: the latter resolves by id OR
                // name (see the lookup above), so it is not a row key.
                //
                // No row yet means a first-ever launch, which has no prior
                // session to resume anyway, so there is nothing for eager
                // resume to gate on and skipping is correct. Failure is
                // logged, never fatal: the launch itself is still valid and
                // falls back to lazy spawn, exactly as it did before.
                match mstore.instance_get(&agent.id) {
                    Ok(Some(existing)) => {
                        let relaunch = crate::backend::storage::store::AgentInstance {
                            block_id: block_id.clone(),
                            ..existing
                        };
                        if let Err(e) = mstore.instance_create(&relaunch) {
                            tracing::warn!(
                                block_id = %block_id,
                                agent_id = %agent.id,
                                error = %e,
                                "agent.open: could not record this launch on the agent's row — \
                                 eager resume will fall back to lazy spawn for this pane"
                            );
                        }
                    }
                    Ok(None) => {}
                    Err(e) => {
                        tracing::warn!(
                            ...
                            agent_id = %agent.id,
                            error = %e,
                            "agent.open: instance lookup failed — eager resume will fall back to lazy"
                        );
                    }
                }
