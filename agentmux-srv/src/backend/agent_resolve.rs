...
mod tests {
    ...
    /// The §9.2 counters are per site and read back sorted. Site names are
    /// unique to this test: the counters are process-wide and tests run in
    /// parallel.
    #[test]
    fn fallback_counters_are_recorded_per_site() {
        let count = |site: &str| {
            uid_fallback_counts()
                .into_iter()
                .find(|(s, _)| *s == site)
        ...
        let before = count("test.counters.b");
        record_uid_fallback("test.counters.b");
        record_uid_fallback("test.counters.b");
        record_uid_fallback("test.counters.a");
        assert_eq!(count("test.counters.b"), before + 2);
        let sites: Vec<&str> = uid_fallback_counts().into_iter().map(|(s, _)| s).collect();
