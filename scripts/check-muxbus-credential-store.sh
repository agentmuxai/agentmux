...
# the per-channel `state.mstore` finds NOTHING whenever the shared root
# resolves, which is the normal case.
#
# THE REGRESSION (reagent P0 on PR #3023): the delivery tier-4 cloud relay
# called `relay_token(source, &state.mstore, ...)`. Every lookup missed,
...
# `mstore`, `id_store` AND `identity_store` with a single in-memory Store, so a
...
# Exit 0 = clean, exit 1 = a muxbus credential call was handed `state.mstore`.

set -euo pipefail

cd "$(dirname "$0")/.."

# Only `state.mstore` is matched, not a bare `mstore` identifier: several
# functions inside `muxbus/` name their own store parameter `mstore` for
...
if [[ -n "$report" ]]; then
    echo "ERROR: muxbus credential lookup against state.mstore." >&2
    echo "" >&2
    ...
    echo "handlers write them. Reading from the per-channel state.mstore finds" >&2
...
echo "OK: no muxbus credential lookup reads from state.mstore."
exit 0
