...
Delegates (in order):
1. `account.key.verify` — stores the secret in the OS keychain (the keychain write is **unconditional** — the secret is stored regardless of whether `validate` is set). If `validate: true`, also runs a live probe against the provider before writing; a probe failure aborts here and returns the error without touching the DB. (Step 1 is non-atomic with respect to steps 2–3; see compensation note below.)
...
S1: `agent_id` in the request must match `ctx.agent_id` — the same two-step check as `identity.account.upsert` (reject if `ctx.agent_id` is empty, then reject on mismatch; see §8).
...
Delegates via: `instance_get_by_name(agent_id)` (storage layer — maps agent slug → `AgentInstance`) → read `memory_id` from the returned row → `getmemory { id: memory_id }`. `CommandGetAgentInstanceData` takes an instance UUID (`id: String`), not a slug, so the handler cannot call `getagentinstance` directly with the request's `agent_id`; `instance_get_by_name` is the correct intermediate.

If `memory_id` is null (no preset bound), fall back in two steps:
1. `listmemories` + filter `is_blank: true` → extract the blank singleton's `id`. (`listmemories` returns summary fields only — `instructions`, `context_files`, `mcp_servers`, and `skills` are absent from its response.)
2. `getmemory { id: <blank_id> }` → returns the full Memory object.

Both steps are required; calling `listmemories` alone is insufficient to satisfy the response contract.
...
| `preset.self.get` | `instance_get_by_name(agent_id)` → `getmemory` | `instance_get_by_name` maps agent slug → instance row → `memory_id`; `getagentinstance` takes a UUID and cannot be used directly; blank-singleton fallback requires `listmemories` (for id) + `getmemory` (for full object) — two calls |
