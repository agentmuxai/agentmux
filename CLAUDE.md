...
| `task package` | **Portable builds.** Builds + packages a local portable to ~/Desktop with an ephemeral, traceable label — NO version bump, NO git mutation. Every build gets a unique stamped folder and a branch-scoped data dir (session persists across rebuilds). `task package -- --fresh` for a throwaway data dir; `task package -- <dir>` for an alternate output dir. The only local portable-build command. | No |
| `task package:local` | Alias of `task package` (now itself ephemeral). Kept for muscle memory. | No |
...
**Data isolation is by channel, not by build.** Local portables key their data dir on `dev-portable-<branch>` (the compile-time `AGENTMUX_BUILD_CHANNEL_DEFAULT`, baked via `option_env!` — `agentmux-common/build.rs` forces a recompile when it changes). So all builds of one branch share a data dir → your test session (agents, panes, auth) survives an iterate-rebuild loop. Different branches are isolated automatically. `task package -- --fresh` suffixes the channel with the build stamp for a one-off clean-slate data dir. Releases use the `stable` channel (the `option_env!` fallback); `task release` / CI never call `task package`.
