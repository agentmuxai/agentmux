...
| `task package:macos` | **Signed DMG builds (macOS).** As of `docs/specs/SPEC_MACOS_DMG_PER_BUILD_CHANNEL_2026_08_24.md`, same per-build channel isolation as `task package`/`task package:linux` — bakes `local-<branch>-<hash>-<build-id>` so the build is its own AgentMux instance, with its own isolated `settings.json` (see the settings-isolation paragraph below — this is what actually fixes a `network:lan_discovery` toggle appearing "on" in a fresh local macOS build). Output: `~/Desktop/AgentMux_<version>_arm64.dmg` (filename stays version-only — unlike Windows/Linux, NOT label-suffixed; see the spec for why). `task package:release:macos` bakes `stable` for release artifacts; requires a `Developer ID Application` cert in the keychain + a `notarytool` keychain profile. | No |
...
Old specs use `AGENTBUS_*` / `agentbus.asaf.cc` — those are historical documents describing the predecessor service. Do not update them; do not introduce new `agentbus` naming.
