...
| `task package` | **Portable builds.** Builds + packages a local portable to ~/Desktop with an ephemeral, traceable label — NO version bump, NO git mutation. Every build gets a unique stamped folder and a **per-build data dir** (its own AgentMux instance — agents + auth carry over globally, pane layout + memories start fresh). `task package -- --fresh` is now a no-op (every build is already isolated); `task package -- <dir>` for an alternate output dir. The only local portable-build command. | No |
